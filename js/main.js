jQuery(function ($) {
	...
	$(window).on('scroll', function () {
		...
		if ($(window).scrollTop() > 50 || $('#home-slider').css("display") == "none") {
			//$('.main-nav').addClass('navbar-fixed-top');
			$('.main-nav').removeClass('navbar-transparent');
			$('.nav-for-pc').addClass('navbar-blue');
		}
	});
...
function setActualDate(element) {
	let now = new Date();
	let day = ("0" + now.getDate()).slice(-2);
	let month = ("0" + (now.getMonth() + 1)).slice(-2);
	document.getElementById(element).value = now.getFullYear() + "-" + month + "-" + day;
}